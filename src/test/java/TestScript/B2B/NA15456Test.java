...
public class NA15456Test extends SuperTestClass {
	...
	String B2BCustomer = "PUNCHOUT-15456";
	...
	public NA15456Test(String store){
		this.Store = store;
		this.testName = "NA-15456";
	}
	...
	public void NA15456(ITestContext ctx){
		try{
			...
			//check
			String CardId=driver.findElement(By.xpath(".//*[@id='orderList']/xmp[1]")).getText().split("cart=\"")[1].split("\"")[0];
			String Description=driver.findElement(By.xpath(".//*[@id='orderList']/xmp[1]")).getText().split("Description")[2];
			String SupplierPartID=driver.findElement(By.xpath(".//*[@id='orderList']/xmp[1]")).getText().split("<SupplierPartID>")[1].split("</SupplierPartID>")[0];
			System.out.println(CardId);
			System.out.println(SupplierPartID);
			if(Description.contains(SupplierPartID) && Description.contains(CardId)){
				Assert.assertEquals(true,true);
			}
			
			changeAribaProductTatal(quantity);
			String quantityTrue=driver.findElement(By.xpath(".//*[@id='orderList']/xmp[1]")).getText().split("quantity=\"")[1]
					.split("\"")[0];
			Assert.assertEquals("3", quantityTrue);
			Dailylog.logInfoDB(8,"Update  Quantity from 1 to 3 and click update button", Store,testName);
		
			
			SimulationToolOCI(UserName,Password,defaultUnit);
			Dailylog.logInfoDB(18,"Go to <B2B Punchout OCI Simulation Tool> on this link", Store,testName);
			saveOCICart(serviceNumber);
			//check
			Assert.assertTrue(punchoutPage.OCI_OrderList.getText().contains(serviceNumber));
			Dailylog.logInfoDB(22,"OCI Click LENOVO CHECKOUT", Store,testName);
			
			
			SimulationToolOracle(23,UserName,Password,defaultUnit);
			...
			//check
			Assert.assertTrue(punchoutPage.OCI_OrderList.getText().contains(serviceNumber));
			Dailylog.logInfoDB(27,"Oracle Click LENOVO CHECKOUT", Store,testName);
