...
public class NA25658Test extends SuperTestClass {
	public B2BPage b2bPage;
	public HMCPage hmcPage;
	public B2BPunchoutPage punchoutPage;
	String oxmlUrl;
	String ociUrl;
	String defaultUnit = "ST_Procter_and_Gamble_CN_01";
	String defaultDMU = "pgcn";
	String AccessLevel = "pgglobal";
	String B2BCustomer = "NA-25658-001";
	String B2BCustomerGroups = "punchOutCustomer";
	int logResult = 0;
	String serviceNumber="5WS0D81063";
	String CardId="";

	public NA25658Test(String store){
		this.Store = store;
		this.testName = "NA-25658";
	}
	
	@Test(alwaysRun = true, groups = {"contentgroup", "b2bpunchout", "p2", "b2b"})
	public void NA25658(ITestContext ctx){
		try{
			this.prepareTest();
			...
			SimulationToolOracle(6,UserName,Password,defaultUnit);
			Dailylog.logInfoDB(8,"Go to B2B Punchout Oracle Simulation Tool on this link:", Store,testName);
			driver.switchTo().frame(driver.findElement(By.id("displayFrame")));
			Common.sleep(8000);
			saveOCICart(serviceNumber);
//			checkExit(driver, punchoutPage, serviceNumber);
			driver.switchTo().defaultContent();
			//check
			Assert.assertTrue(driver.findElement(By.xpath(".//*[@id='orderList']/xmp[1]")).getText().contains(serviceNumber));	
			Assert.assertEquals(true, driver.findElement(By.xpath(".//*[@id='orderList']/xmp[1]")).getText().contains("CNY"));
			Dailylog.logInfoDB(9,"Purchase one product and check cartMessage", Store,testName);
	
			SimulationToolOCI(UserName,Password,defaultUnit,"Post");
			//check
			Dailylog.logInfoDB(13,"Purchase one product and check cartMessage", Store,testName);
//			checkExit(driver, punchoutPage, serviceNumber);
			saveOCICart(serviceNumber);
			Common.sleep(5000);
			switchToWindow(0);
			Assert.assertTrue(punchoutPage.OCI_OrderList.getText().contains(serviceNumber));
			Assert.assertTrue(punchoutPage.OCI_OrderList.getText().contains("CNY"));
		}catch (Throwable e) {
	...
	public void SimulationToolOCI(String UserName,String Password,String defaultUnit,String requestMethod){
		...
		if(requestMethod=="Post"){
			driver.findElement(By.xpath("html/body/table/tbody/tr[3]/td/input[1]"));	
		}else{
			driver.findElement(By.xpath("html/body/table/tbody/tr[3]/td/input[2]"));
		}
	...
	public void creatB2BCustomer(String B2BCustomer,String group, String customerType){
		...
		Common.waitElementClickable(driver, driver.findElement(By.xpath(".//*[contains(@id,'_ajaxselect_"+defaultDMU+"')][text()='"+defaultDMU+"']")), 15);
		driver.findElement(By.xpath(".//*[contains(@id,'_ajaxselect_"+defaultDMU+"')][text()='"+defaultDMU+"']")).click();
	...
	public void creatPunchoutProfile(String ProfileName,String B2BCustomer) throws InterruptedException{
		...
		//NA26568
		driver.findElement(By.id("Content/BooleanEditor[in Content/Attribute[.currencyRmb]]_checkbox")).click();
