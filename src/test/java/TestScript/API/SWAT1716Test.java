...
public class SWAT1716Test extends ServiceSuperTestClass {
	...
	public SWAT1716Test(String store, String userid,
		...
		this.updatedStatus = changedStatus;
		this.updatedByEmail=updatedBy;
		this.expectedStatus = expectedstatus;
	
		this.testName = "SWAT1716";
		super.serviceName = "updateStatus";
	}
	...
	public void DemoScriptRun(ITestContext ctx) {
		try {
			...
			super.paraString = "Order Number is "+HybrisOrderNumber+"; Status updated is ="+updatedStatus+";Updated by "+updatedByEmail;
			...
			if(updatedStatus.equals("Reactivate01")){
				String suspendURL=testData.envData.getSubScriptionUpdateStatusDomain()+MSItemID+"&status=Suspend01"+parameter;
				String suspendResult= hCommon.verifyServiceStatusPOST(suspendURL);
			}
			String uptateStatusURL=testData.envData.getSubScriptionUpdateStatusDomain()+MSItemID+"&status="+updatedStatus+parameter;
		
			super.serviceStatus = hCommon.verifyServiceStatusPOST(uptateStatusURL);
			...
			httpResult = hCommon.HttpRequest(orderserviceURL);
			 MSGetOrderContent=hCommon.getJsonValue(httpResult, "content");
				 ...
				 String MSItemStatus=JCommon.getJSONArrayValue(MSGetOrderContent, 0, "status");
				 assert MSItemStatus.equals(expectedStatus):"Actual status is "+MSItemStatus;
				 ...
				 if(updatedStatus.equals("Suspend01")){
					 String suspendURL=testData.envData.getSubScriptionUpdateStatusDomain()+MSItemID+"&status=Reactivate01"+parameter;
						String suspendResult= hCommon.verifyServiceStatusPOST(suspendURL);
