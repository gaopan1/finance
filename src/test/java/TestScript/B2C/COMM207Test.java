...
public class COMM207Test extends SuperTestClass {
	B2CPage b2cPage;
	HMCPage hmcPage;
	String SubSeries;
	PartSalesPage PSPage;
	private String subproNUM = "moc_888014201";
	...
	private String ValidMerchant_ID = "LENOVOUS02";
	private String InvalidMerchant_ID = "LENOVOUS021";
	private Actions act;

	public COMM207Test(String store){
		this.Store = store;
		this.testName = "COMM-207";
		
	}
	
	@Test(priority = 0, enabled = true, alwaysRun = true, groups = {"commercegroup", "cartcheckout", "p2", "b2c"})
	public void COMM207(ITestContext ctx) {
		try {
			this.prepareTest();
...
//			addMTMQuick();
			...
			Assert.assertTrue(Common.checkElementDisplays(driver, entries, 5));
			
			//step~13
			Dailylog.logInfoDB(13, "Right click entries", Store, testName);
			Common.doubleClick(driver, driver.findElement(entries));
			...
			Assert.assertEquals(TransacationsStatus, "REJECTED");
			WebElement ele_giantType = driver.findElement(By.xpath("//*[contains(@id,'PaymentTransactionEntry.giantType]]_select')]/option[@value='3']"));
	...
	public void changeValueMerchantID(String Merchant_ID){
		...
		Common.sleep(3000);
		WebElement CreditCart_XiPay = 
				driver.findElement(By.id("Content/GenericReferenceEditor[in Content/Attribute[B2CUnit.XiPaySetting]]_div"));
		Common.rightClick(driver, CreditCart_XiPay);
		Common.sleep(2000);
		driver.findElement(By.id("Content/GenericReferenceEditor[in Content/Attribute[B2CUnit.XiPaySetting]]_!open_editor_internal_true_icon_td")).click();
	...
	public boolean checkprice(){
		...
		price_SUBAdd = B2CCommon.GetPriceValue(priceSUBAdd);
