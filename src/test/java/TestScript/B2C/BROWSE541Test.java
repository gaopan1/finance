...
public class BROWSE541Test extends SuperTestClass {
	B2CPage b2cPage;
	HMCPage hmcPage;

	public BROWSE541Test(String store) {
		this.Store = store;
		this.testName = "BROWSE-541";
	}

	@Test(priority = 0, enabled = true, alwaysRun = true, groups = {"browsegroup","uxui",  "p2", "b2c"})
	public void BROWSE541(ITestContext ctx) {
		try {
			this.prepareTest();
			...
			for (int n = 1; n <= aList3.size(); n++) {
				...
				//verify the url
				String accessories_Page_url = driver.getCurrentUrl().toString();
				Assert.assertTrue(!accessories_Page_url.contains("?q="), "the category url cannot right");
				System.out.println("The category url is" + accessories_Page_url);
			}
