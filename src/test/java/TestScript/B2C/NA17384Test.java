...
public class NA17384Test extends SuperTestClass{
	private HMCPage hmcPage;
	private B2CPage b2cPage;
	private String partnumber;
	private String cartName = Common.getDateTimeString();

	public NA17384Test(String store,String partnumber) {
		this.Store = store;
		this.testName = "NA-17384";
		this.partnumber = partnumber;
	}

	@Test(alwaysRun = true,groups = {"accountgroup", "uxui", "p2", "b2c"})
	public void NA17384(ITestContext ctx) throws Exception {
		try{
			this.prepareTest();
			...
			Common.sleep(1500);
			String laptopPageURL1 = driver.getCurrentUrl();
			B2CCommon.addToCartB2C(1, driver, laptopPageURL1, customiseButton);
			...
			//=========== Step:13 Set Product Marketing status as "TempUnavailable" ===========//
...
//			cleanProductCache();
			...
			if(Common.checkElementExists(driver, b2cPage.Save_Cart_redbanner, 5)) {
				...
				Assert.assertTrue(soldOutMsg.contains("TempUnavailable"));
				Dailylog.logInfoDB(14, "Product status after setting the status to 'TempUnavailable' :" + soldOutMsg, Store, testName);
			}
			Common.javascriptClick(driver, b2cPage.lenovo_checkout);
			Common.sleep(2000);

			//=========== Step:15 Check the pop up after clicking on 'Checkout' button ===========//
			if(Common.checkElementDisplays(driver, By.xpath(ViewSavedQuote_SoldOutPopUpMsg), 5)) {
