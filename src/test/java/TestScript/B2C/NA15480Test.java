...
public class NA15480Test extends SuperTestClass {

	public NA15480Test(String store) {
		this.Store = store;
		this.testName = "NA-15480";
	}

	@Test(priority = 0, enabled = true, alwaysRun = true, groups = {"accountgroup", "account", "p1", "b2c" ,"compatibility"})
	public void NA15480(ITestContext ctx) {
		try {
			this.prepareTest();
...
//			if (Common.checkElementExists(driver, b2cPage.Navigation_SignInLink, 5))
//				b2cPage.Navigation_SignInLink.click();
//			else
...
//			driver.get(testData.B2C.getloginPageUrl());
			...
			Dailylog.logInfoDB(1, "Validate all default errors", Store, testName);
			...
			Dailylog.logInfoDB(2, "Validate Email format error", Store, testName);
			...
			Dailylog.logInfoDB(3, "alidate Email Confirmation error", Store, testName);
			...
			Dailylog.logInfoDB(4, " Input Password less than 8 characters", Store, testName);
			...
			Dailylog.logInfoDB(5, " Input Password less than 8 characters Input different password in the confirm password field", Store, testName);
			...
			Dailylog.logInfoDB(6, "Input already registered email ID", Store, testName);
			...
			Dailylog.logInfoDB(7, "Input all of the valid details then click create account", Store, testName);
			...
			Dailylog.logInfoDB(8, "click sign into my account button, and go to log in page, input new user ID and password without activate account", Store, testName);
			...
			// Active account
			Dailylog.logInfoDB(9, "Active account", Store, testName);
			...
			Dailylog.logInfoDB(10, "Log in with new account", Store, testName);
...
//				b2cPage.Navigation_SignOutLink.click();
			...
			Dailylog.logInfoDB(10, "Log in with invalid password", Store, testName);
