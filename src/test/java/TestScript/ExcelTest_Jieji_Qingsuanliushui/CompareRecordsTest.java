package TestScript.ExcelTest_Jieji_Qingsuanliushui;
...
public class CompareRecordsTest {
	...
	ArrayList<String> list_Qingsuan_left = new ArrayList<String>();
	ArrayList<String> list_Jiejiliushui_left = new ArrayList<String>();
	
	
	public CompareRecordsTest(String sourceFile,
							String destinationFile,
							String qingsuanliushui_sheet,
							String jiejiliushui_sheet,
							String result_SheetName,
	...
	public void test() throws Exception{
        ...
        System.out.println("colLength1 is :" + colLength1);
		 ...
		 Dailylog.logInfo("set size is ：" + set.size());
		 ...
		 for(int x = 0; x<list_Qingsuan.size();x++){
			 ...
			 String str = list_Qingsuan.get(x);
			 
			 if(!common.contains(str)){
				list_Qingsuan_left.add(str);
			 }	 
		 }
		
		for(int y = 0; y<list_Jiejiliushui.size();y++){
			
			String str = list_Jiejiliushui.get(y);
			
			if(!common.contains(str)){
				list_Jiejiliushui_left.add(str);
			}	
		}
...
//		createNewExcels(destinationTable);
		...
		try{
			...
			cell.setCellValue("清算流水");
	
			HSSFCell cell2 = row1.createCell(1);
			cell2.setCellValue("接机流水");
			...
			for(int t = 1; t<=left_zhifu_list.size();t++){
				...
				if(t==left_zhifu_list.size()){
					...
					cell.setCellValue(pay_total);
					row1 = sheet.createRow(t+2);
					
					cell = row1.createCell(6);
					
					cell.setCellValue(account_total);
				}
			}
